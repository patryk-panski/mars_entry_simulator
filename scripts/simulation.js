...
const lander = {
  ...
  meshgrid(x, y) {
   //define function meshgrid (same as in MatLab) - returns X, Y matrices. Used for defineMesh
    ...
    let temp = []; //temporary array to store rows of Y matrix
  ...
  defineMesh() {
     ...
     */
     ...
     //variables from the lander object
       let { rBody, rNose, rShoulder, theta } = this;


     //define dimensions for nose, frustum and shoulder
       //nose
       let noseArea = Math.PI * (rNose * Math.cos(theta)) * (rNose * Math.cos(theta));//area of projected nose [m^2] //UNUSED
       let noseH = rNose * (1 - Math.sin(theta)); //X-coordinate where nose edge meets frustum [m]
       let noseL = rNose * Math.cos(theta); //R-coordinate where nose edge meets frustum [m]
       let noseCap = 2 * Math.PI * rNose * noseH; //area of a spherical cap [m^2] //UNUSED

       //shoulder height and length
       let shoulderH = rShoulder * (1 - Math.cos(theta));
       let shoulderL = rShoulder * Math.sin(theta);

       //frustrum
       let frustumEdgeX2 = noseH + (rBody - noseL - shoulderH) / Math.tan(theta); //X-coordinate where frustum edge meets shoulder [m] //UNUSED
       let frustumEdgeR2 = rBody - shoulderH; //R-coordinate where frustum edge meets shoulder [m]
       let frustumSide = Math.sqrt((rBody - noseL) * (rBody - noseL) + (frustumEdgeR2 - noseL) * (frustumEdgeR2 - noseL)); //Q: Is this correct?
       let frustumArea = Math.PI * (rBody + noseL) * frustumSide; //UNUSED
       let frustumL = rBody - noseL - shoulderH; //length
       let frustumH = frustumL / Math.tan(theta); //height
       let gradient = frustumH / frustumL; //UNUSED
       //shoulderStartR and shoulderStartX same as frustumEdgeX2 and frustumEdgeR2
       //cone (if the geometry is extended)
       let coneH = (rBody - shoulderH) / Math.tan(theta); //cone height
       let coneVertex = - coneH + noseH + frustumH;
     ...
     let z = numeric.mul(rhoCM, numeric.sin(thetaCM)); //z = rhoC.*sin(thetaC)
...
const Mars = {
  ...
  omega: 7.08824 * Math.pow(10, -5), //Mars rotation rate [rad/s]
  mu: 4.282837 * Math.pow(10, 13), //Mars standard gravitational parameter - GM - [m^3/s^2]
  J2: 1.960454 * Math.pow(10, -3), //Mars second dynamic factor
  //coefficients to calculate convective and radiative heat rate
  k: 1.898 * Math.pow(10, -4), //Mars Sutton&Graves coeff.
  C: 2.35 * Math.pow(10, 4), //Mars Tauber&Sutton coeff. C
...
const traj = (function (lander) {
  ...
  function calcAtmoProperties(h, vInf, lander) {
    ...
    let diameter = lander.diameter;

    //atmosphere conditions
    let atm = mission[current];

    let tInf, pInf, densInf, cpInf, viscInf;
    ...
    //calculate freestream properties
      let R = pInf / (densInf * tInf); //gas constant R
      let cvInf = cpInf - R; //specific heat at constant volume Cv
      let gamma = cpInf / cvInf; //ratio of specific heats
      let aSound = Math.sqrt(gamma * R * tInf); //speed of sound
      let qInf = 0.5 * densInf * vInf * vInf; //dynamic pressure
      let mach = vInf / aSound; //Mach number
      let Re = densInf * vInf * diameter / viscInf; //Reynolds Number
      let Kn = mach / Re * Math.sqrt(Math.PI * gamma / 2); //Knudsen Number
      let cpMax = (2 / (mach * mach * gamma)) * (Math.pow(((gamma + 1) * (gamma + 1) * mach * mach) / (4 * gamma * mach * mach - 2 * (gamma - 1)), (gamma / (gamma - 1))) * ((1 - gamma + 2 * gamma * mach * mach) / (gamma + 1)) - 1);
  ...
  function calcAerodynamics(x, y, z, vInfWind, cpMax, qInf, pInf, alpha, omega) {
    ...
    let xVector = [1, 0, 0], yVector = [0, 1, 0], zVector = [0, 0, 1];

    //variables from lander object
    let { mass, diameter, refArea, COMpayload, rPayload, heightPayload, hsThickness, hsDensity } = lander;

    //calculate detailed geometry of the mesh, forces, moments etc.
    //define whole matrices refilled after each inner loop
    let dim = x.length - 1;
    let
      areas = new Array(dim), volumes = new Array(dim), masses = new Array(dim), normalSX = new Array(dim), normalSY = new Array(dim), normalSZ = new Array(dim), t1locX = new Array(dim), t1locY = new Array(dim), t1locZ = new Array(dim), t2locX = new Array(dim), t2locY = new Array(dim), t2locZ = new Array(dim), locX = new Array(dim), locY = new Array(dim), locZ = new Array(dim);
    let
      thetasX = new Array(dim), thetasY = new Array(dim), thetasZ = new Array(dim), thetasV = new Array(dim), hsAngle = new Array(dim), cps = new Array(dim), pressures = new Array(dim), forces = new Array(dim), forcesX = new Array(dim), forcesY = new Array(dim), forcesZ = new Array(dim), forcesV = new Array(dim), massMomentsX = new Array(dim), massMomentsY = new Array(dim), massMomentsZ = new Array(dim);

    for (let i = 0; i < dim; i++) {
      //create temporary rows, refilled after each loop
      let
        areasR = new Array(dim), volumesR = new Array(dim), massesR = new Array(dim), normalSXR = new Array(dim), normalSYR = new Array(dim), normalSZR = new Array(dim), t1locXR = new Array(dim), t1locYR = new Array(dim), t1locZR = new Array(dim), t2locXR = new Array(dim), t2locYR = new Array(dim), t2locZR = new Array(dim), locXR = new Array(dim), locYR = new Array(dim), locZR = new Array(dim);
      let
        thetasXR = new Array(dim), thetasYR = new Array(dim), thetasZR = new Array(dim), thetasVR = new Array(dim), hsAngleR = new Array(dim), cpsR = new Array(dim), pressuresR = new Array(dim), forcesR = new Array(dim), forcesXR = new Array(dim), forcesYR = new Array(dim), forcesZR = new Array(dim), forcesVR = new Array(dim), massMomentsXR = new Array(dim), massMomentsYR = new Array(dim), massMomentsZR = new Array(dim);

      for (let j = 0; j < dim; j++) {
        let vector1 = [x[i][j], y[i][j], z[i][j]]; //define first corner of trapezoid
        let vector2 = [x[i+1][j], y[i+1][j], z[i+1][j]]; //define second corner of trapezoid
        let vector3 = [x[i][j+1], y[i][j+1], z[i][j+1]]; //define third corner of trapezoid
        let vector4 = [x[i+1][j+1], y[i+1][j+1], z[i+1][j+1]]; //define fourth corner of trapezoid
        let side1 = numeric.sub(vector2, vector1); //define side vector of trapezoid
        let length1 = numeric.norm2(side1); //calculate side length of trapezoid
        let side2 = numeric.sub(vector3, vector1); //define top vector of trapezoid
        let length2 = numeric.norm2(side2); //calculate top length of trapezoid
        let side3 = numeric.sub(vector4, vector2); //define base vector of trapezoid
        let length3 = numeric.norm2(side3); //calculate base length of trapezoid
        let height = Math.sqrt(length1 * length1 - 0.25 * (length3 - length2) * (length3 - length2)); //calculate isosceles trapezoid height

        let cross1 = numeric.sub(vector2, vector3); //define diagonal vector 1 across trapezoid
        let cross2 = numeric.sub(vector4, vector1); //define diagonal vector 2 across trapezoid
        ...
        let t1Area = 0.5 * length3 * height; //area of triangle 1
        let t2Area = 0.5 * length2 * height; //area of triangle 2
    ...
    //calculate total forces, masses and COM
      let totalForcesX = numeric.sum(forcesX);
      let totalForcesY = numeric.sum(forcesY);
      let totalForcesZ = numeric.sum(forcesZ);
      let massHS = numeric.sum(masses); //heatshield mass (kg)
      let massPL = mass - massHS; //payload mass
      let COMx = (numeric.sum(massMomentsX) + massPL * COMpayload[0]) / mass; //X-coord of centre of mass
      let COMy = (numeric.sum(massMomentsY) + massPL * COMpayload[1]) / mass; //Y-coord of centre of mass
      let COMz = (numeric.sum(massMomentsZ) + massPL * COMpayload[2]) / mass; //Z-coord of centre of mass
      let COM = [COMx, COMy, COMz]; //centre of mass vector //UNUSED

    //find moments of inertia and moments about centre of mass
    //define whole matrices refilled after each inner loop
      let Ixx = new Array(dim), Iyy = new Array(dim), Izz = new Array(dim), Ixy = new Array(dim), Ixz = new Array(dim), Iyz = new Array(dim),
      forceMomentsX = new Array(dim), forceMomentsY = new Array(dim), forceMomentsZ = new Array(dim); //moments of force about the centre of mass

    for (let i = 0; i < dim; i++) {
      //preallocate rows for the matrices
      let IxxR = new Array(dim), IyyR = new Array(dim), IzzR = new Array(dim), IxyR = new Array(dim), IxzR = new Array(dim), IyzR = new Array(dim),
      forceMomentsXR = new Array(dim), forceMomentsYR = new Array(dim), forceMomentsZR = new Array(dim);
    ...
    //create moment of inertia matrix and force moments vector
      let totalIxx = numeric.sum(Ixx) + 0.1535 + massPL * rPayload * rPayload / 2 + massPL * (COMpayload[0] - COMx) * (COMpayload[0] - COMx);
      let totalIyy = numeric.sum(Iyy) - 0.4664 + massPL / 12 * (3 * rPayload * rPayload + heightPayload * heightPayload) + massPL * (COMpayload[1] - COMy) * (COMpayload[1] - COMy);
      let totalIzz = numeric.sum(Izz) + 0.6236 + massPL / 12 * (3 * rPayload * rPayload + heightPayload * heightPayload) + massPL * (COMpayload[2] - COMz) * (COMpayload[2] - COMz);
      let totalIxy = numeric.sum(Ixy) - 0.3424;
      let totalIxz = numeric.sum(Ixz) + 0.1486;
      let totalIyz = numeric.sum(Iyz) + 1.4477;
      let totalFMX = numeric.sum(forceMomentsX); //summing moments of force
      let totalFMY = numeric.sum(forceMomentsY);
      let totalFMZ = numeric.sum(forceMomentsZ);
      let totalFM = [totalFMX, totalFMY, totalFMZ];
      let IMatrix = [[totalIxx, totalIxy, totalIxz], [totalIxy, totalIyy, totalIyz], [totalIxz, totalIyz, totalIzz]]; //create moment of inertia matrix

    //calculate angular accelerations/omega dots
      let angAccels = numeric.solve(IMatrix, numeric.sub(totalFM, cross(omega, numeric.dot(IMatrix, omega))));

    //calculate centres of pressure
      let COPx = - totalFMY / totalForcesZ;
      let COPy = totalFMX / totalForcesZ;
      let COPz = totalFMY / totalForcesX;
      let COP = [COPx, COPy, COPz]; //UNUSED

    //calculate aerodynamic coefficients
      let A = - totalForcesX;
      let N = - totalForcesZ;
      let Y = totalForcesY;
      let Ca = A / (qInf * refArea); //axial force coefficient
      let Cn = N / (qInf * refArea); //normal force coefficient
      let Cy = Y / (qInf * refArea); //side force coefficient //UNUSED
      let Cl = Cn * Math.cos(alpha) - Ca * Math.sin(alpha); //lift coefficient
      let Cd = Cn * Math.sin(alpha) + Ca * Math.cos(alpha); //drag coefficient
      let Croll = totalFMX / (qInf * refArea * diameter); //roll moment coefficient
      let Cpitch = totalFMY / (qInf * refArea * diameter); //pitch moment coefficient
      let Cyaw = totalFMZ / (qInf * refArea * diameter); //yaw moment coefficient
      let LDRatio = Cl / Cd;
  ...
  function calcTrajChange(t, variables) {
    ...
    let { mass, rNose, refArea } = lander;

    //define major trajectory variables from the input
    let [ r, lat, lon, u, v, w, e0, e1, e2, e3, omegaX, omegaY, omegaZ ] = variables;

    //calculate other trajectory variables from the input
    let h = r - Mars.r;
    let vInfGc = [u, v, w];
    let vInf = Math.sqrt(u * u + v * v + w * w);
    let FPA = Math.asin(w / vInf);
    let azi = Math.atan2(v, u); //asin(v/Vinf*cos(FPA));
    let roll = Math.atan2(2 * e0 * e1 + 2 * e2 * e3, e0 * e0 - e1 * e1 - e2 * e2 + e3 * e3);
    let pitch = - Math.asin(2 * (e1 * e3 - e0 * e2));  //1,2,3 Quaternion to Euler angle conversion (Diebel, 2006)
    let yaw = Math.atan2(2 * e1 * e2 + 2 * e0 * e3, e0 * e0 + e1 * e1 - e2 * e2 - e3 * e3);
    let omega = [omegaX, omegaY, omegaZ];

    //set up rotation matrices from quaternions
    //transformation matrix from geodetic to body frame (Karlgaard, 2006)
    let Ggd2b = [[e0 * e0 + e1 * e1 - e2 * e2 - e3 * e3, 2 * (e1 * e2 + e0 * e3), 2 * (e1 * e3 - e0 * e2)], [2 * (e1 * e2 - e0 * e3), e0 * e0 - e1 * e1 + e2 * e2 - e3 * e3, 2 * (e0 * e1 + e2 * e3)], [2 * (e1 * e3 + e0 * e2), 2 * (e2 * e3 - e0 * e1), e0 * e0 - e1 * e1 - e2 * e2 + e3 * e3]];
    //transformation matrix from body to geodetic frame
    let Gb2gd = numeric.transpose(Ggd2b); //UNUSED
    let Ggc2gd = [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
    //assuming Mars is spherical, there is no difference between geocentric and geodetic
    let Ggc2b = numeric.dot(Ggc2gd, Ggd2b);
    let Gb2gc = numeric.transpose(Ggc2b); //UNUSED

    //define velocities and angles in body frame from input
    let vInfB = numeric.dot(Ggc2b, vInfGc); //velocity vector in body frame MxV
    let [ vX, vY, vZ ] = vInfB; //x, y, z body frame velocity coordinates

    let beta = Math.atan2(vY, Math.sqrt(vX * vX + vZ * vZ)); //sideslip angle
    let alpha = Math.atan2(vZ, vX); //angle of attack
    let bank = Math.atan2(2 * (e0 * e1 + e2 * e3) + Math.sin(beta) * Math.sin(-FPA), ((e0 * e0 - e1 * e1 + e2 * e2 - e3 * e3) * Math.cos(azi) - (2 * (e1 * e2 - e0 * e3)) * Math.sin(azi)) * Math.cos(-FPA)); //bank angle //UNUSED
    let vInfWind = [vInf * Math.cos(beta) * Math.cos(alpha), vInf * Math.sin(beta), vInf * Math.cos(beta) * Math.sin(alpha)]; //wind velocity vector in body frame
    let alphaTotal = Math.acos(Math.cos(alpha) * Math.cos(beta));
    let phi_a = Math.atan2(Math.tan(beta), Math.sin(alpha));

    //calculate atmospheric properties
    let [ tInf, pInf, densInf, cpInf, viscInf, R, cvInf, gamma, aSound, qInf, mach, Re, Kn, cpMax ] = calcAtmoProperties(h, vInf, lander);

    //define mesh coordinates
    let { x, y, z } = lander.mesh;

    //calculate aerodynamic coefficients and angular accelerations
    let [ Cl, Cd, , , , ,angAccels ] = calcAerodynamics(x, y, z, vInfWind, cpMax, qInf, pInf, alpha, omega);
    let [ omegaXDot, omegaYDot, omegaZDot ] = angAccels;

    //calculate heating values
    let qConv = Mars.k * Math.sqrt(densInf / rNose) * vInf * vInf * vInf / 10000; // Sutton-Graves relation for convective heat flux at stagnation point [W/cm2]
    let qRad;
    if (vInf > 6500) {
      let TSv = numeric.spline(TauberSuttonVelocityFunction.velocity, TauberSuttonVelocityFunction.fv).at(vInf);
      qRad = Mars.C * Math.pow(rNose, Mars.a) * Math.pow(densInf, Mars.b) * TSv; //Tauber-Sutton relation for radiative heat flux at stagnation point [W/cm2]
    } else {
      qRad = 0;
    }
    let qTotal = qConv + qRad; //UNUSED
    let tempNose = Math.pow(qTotal / (5.670373e-12), 0.25);

    //calculate aerodynamic forces and decelerations
      //forces
        let drag = Cd * qInf * refArea; //drag force [N]
        let lift = Cl * qInf * refArea; //lift force [N]
        let BC = mass / (Cd * refArea); //ballistic coefficient [kg/m2]
      //drag components //UNUSED
        let Du = - drag * Math.cos(FPA) * Math.cos(azi); //component of drag force in N direction [N]
        let Dv = - drag * Math.cos(FPA) * Math.sin(azi); //component of drag force in E direction [N]
        let Dw = - drag * Math.sin(FPA); //component of drag force in D direction [N]
        let Dgc = [Du, Dv, Dw];
        let Dbody = numeric.dot(Ggc2b,Dgc); //UNUSED
      //lift components
        let Lu = lift * Math.sin(FPA) * Math.cos(azi); //component of lift force in N direction [N]
        let Lv = lift * Math.sin(FPA) * Math.sin(azi); //component of lift force in E direction [N]
        let Lw = - lift * Math.cos(FPA); //component of lift force in D direction [N]
        let Lgc = [Lu, Lv, Lw];
        let Lbody = numeric.dot(Ggc2b, Lgc); //UNUSED
      //decelerations
        let au = (Du + Lu) / mass;  //component of aerodynamic acceleration in N direction [m/s2]
        let av = (Dv + Lv) / mass;  //component of aerodynamic acceleration in E direction [m/s2]
        let aw = (Dw + Lw) / mass;  //component of aerodynamic acceleration in D direction [m/s2]
        let a = Math.sqrt(au * au + av * av + aw * aw);  //total aerodynamic acceleration
        let gload = a / 9.81;  //total g-load imparted to vehicle //UNUSED
        let agc = [au, av, aw];
        let abody = numeric.dot(Ggc2b, agc); //UNUSED
      //gravitational model
        let J = 3 / 2 * Mars.J2 * Mars.r * Mars.r; //Wagner (1970) definition
        let gu = - (Mars.mu * J * Math.sin(2 * lat) / (r * r * r * r)); //component of gravitational acceleration in N direction [m/s2] Wagner (1970)
        let gv = 0; //component of gravitational acceleration in E direction [m/s2]
        let gw = (Mars.mu / (r * r)) - (Mars.mu * (2 - 3 * Math.cos(lat) * Math.cos(lat)) / (r * r * r * r)); //component of gravitational acceleration in D direction [m/s2]
        let g = Math.sqrt(gu * gu + gv * gv + gw * gw); //total gravitational acceleration //UNUSED
        let ggc = [gu, gv, gw];
        let gbody = numeric.dot(Ggc2b, ggc); //UNUSED

    //run equations of motion
    let rdot = - w; //rate of change of r [m/s] (Karlgaard, 2009)
    let latdot = u / r; //rate of change of latitude [rad/s]
    let londot = v / (r * Math.cos(lat)) - Mars.omega; //rate of change of longitude [rad/s] POSITIVE FOR RETROGRADE
    let udot = au + (1 / r) * (u * w - v * v * Math.tan(lat)) + gu - 2 * Mars.omega * v * Math.sin(lat); //acceleration in N direction [m/s2] (Karlgaard, 2009) (added Coriolis + centrifugal)
    let vdot = av + (1 / r) * (u * v * Math.tan(lat) + v * w) + gv + 2 * Mars.omega * (w * Math.cos(lat) + u * Math.sin(lat)); //acceleration in E direction [m/s2] (added Coriolis + centrifugal)
    let wdot = aw - (1 / r) * (u * u + v * v) + gw - 2 * Mars.omega * v * Math.cos(lat); //acceleration in D direction [m/s2] (added Coriolis + centrifugal)
    let dec = Math.sqrt((udot - gu) * (udot - gu) + (vdot - gv) * (vdot - gv) + (wdot - gw) * (wdot - gw)); //total deceleration //UNUSED

    //attitude dot (quaternions)
    let multiplic = numeric.sub(omega, numeric.mul((1 / r), numeric.dot(Ggc2b, [v, - u, - v * Math.tan(lat)])));
    let e0dot = 0.5*numeric.dot([- e1, - e2, - e3], multiplic);
    let e1dot = 0.5*numeric.dot([e0, - e3, e2], multiplic);
    let e2dot = 0.5*numeric.dot([e3, e0, - e1], multiplic);
    let e3dot = 0.5*numeric.dot([- e2, e1, e0], multiplic);
  ...
  function solve(tMax = 350, precision = 1e-6, numberOfSteps = 15000) {
    ...
    //define initial conditions
    let var0 = [cond.r, cond.lat, cond.lon, cond.u, cond.v, cond.w, cond.e0, cond.e1, cond.e2, cond.e3, cond.omegaX, cond.omegaY, cond.omegaZ];

    //define event function for the ODE solver - stops integration when mach number reaches 2.5
    function eventF(t,y) {
      let h = y[0] - Mars.r;
      let vInf = Math.sqrt(y[3] * y[3] + y[4] * y[4] + y[5] * y[5]);
      let mach = calcAtmoProperties(h, vInf, lander)[10];
      let qInf = calcAtmoProperties(h, vInf, lander)[9];
...
const results = {
  ...
  createArrays(sol, arr, scale) {
     ...
     */
    //initialize
    let quaternionNED = new THREE.Quaternion();
    let rotationNED = new THREE.Matrix4();
    let solLength = sol.x.length;

    //preallocate arrays
    let time = sol.x;
    let alt = new Array(solLength);
    let pos = new Array(3 * solLength);
    let rotN = new Array(4 * solLength);
    let rotB = new Array(4 * solLength);
    let xVel = new Array(solLength);
    let yVel = new Array(solLength);
    let zVel = new Array(solLength);
    let vel = new Array(solLength);
    let qInf = new Array(solLength);
    let tempNose = new Array(solLength);
    let AoA = new Array(solLength);

    //create arrays
    for (let i = 0; i < solLength; i++) {

      let [ r, lat, lon, u, v, w, e0, e1, e2, e3 ] = sol.y[i];
      ...
      let densInf = traj.calcAtmoProperties(alt[i], vel[i], lander)[2];
      let qConv = Mars.k * Math.sqrt(densInf / lander.rNose) * vel[i] * vel[i] * vel[i] / 10000; // Sutton-Graves relation for convective heat flux at stagnation point [W/cm2]
      let qRad;
      if (vel[i] > 6500) {
        let TSv = numeric.spline(traj.TauberSuttonVelocityFunction.velocity, traj.TauberSuttonVelocityFunction.fv).at(vel[i]);
        qRad = Mars.C * Math.pow(lander.rNose, Mars.a) * Math.pow(densInf, Mars.b) * TSv; //Tauber-Sutton relation for radiative heat flux at stagnation point [W/cm2]
      } else {
        qRad = 0;
      }
      let qTotal = qConv + qRad;
      tempNose[i] = Math.pow(qTotal / (5.670373e-12), 0.25);
...
const livePlots = {
  ...
  options1: {
   //define initial options
    layout: {
      padding: {
        left: 10, right: 25, top: 25, bottom: 5 //padding of the graph
      }
    },
  ...
  init() {
    ...
    let ctx1 = document.getElementById("plot1");
    let ctx2 = document.getElementById("plot2");
...
const liveOutputs = {
  ...
  update(time, { spline }) {
    /**
     * Updates altitude, velocity and dynamic pressure outputs in the render loop.
     *
     * @param {number} time - real simulation time
     * @param {object} spline - splines from result object
     */
    ...
    //introduce game logic for the update of live outputs
    if (altitude > 8000 && mach > 1.1 && mach < 2.2 && dynamicPress < 850 && dynamicPress > 239) {
      alert("Parachute can be safely deployed at this point! Congratulations!");
...
//TODO make a separate module with game logics that is called inside render loop and it accepts current results as arguments
...
const G = (function() {
 //initialize variables
  ...
  let mixer, clipAction, isPlay = false;
  ...
  function initScene() {
    ...
    //get NEDframe
    let NEDframe = scene.getObjectByName('NED');
  ...
  */
  function createMars(lander, Mars, manager) {
    ...
    let scale = lander.mesh.scale;
    let radius = Mars.r / scale;

    //create PLANET FRAME coordinate system
    let PLANETframe = createFrame('PLANET', radius + 10);

    //create all 8 sphere parts
    let mars = new THREE.Group(); //container for all sphere parts
    let texture, material, geometry, mesh;
    let loader = new THREE.TextureLoader(manager); //loader for textures
    ...
    PLANETframe.traverse(function(obj){obj.matrixAutoUpdate = false;});
  ...
  */
  function createLander(lander, cond) {
    ...
    //define initial conditions
    let { lat, lon, r, e0, e1, e2, e3 } = cond;

    //create mesh group for BODY FRAME
    let BODYframe = createFrame('BODY', 0.15);

    //create mesh group for NED FRAME
    let NEDframe = createFrame('NED', 25);
    ...
    let geometry = new THREE.LatheGeometry(points, rLath.length - 1);
    let material = new THREE.MeshBasicMaterial({ color: 0x708090, wireframe: true, transparent: true, side: THREE.DoubleSide });
    ...
    //using properties
    let xPLANET = r / scale * Math.cos(lat) * Math.cos(lon);
    let yPLANET = r / scale * Math.cos(lat) * Math.sin(lon);
    let zPLANET = r / scale * Math.sin(lat);
  ...
  function setLander(lander, cond) {
    ...
    let oldNEDframe = scene.getObjectByName('NED');
  ...
  function setLanderPosition(lander, cond) {
    ...
    let x = r/scale * Math.cos(lat) * Math.cos(lon);
    let y = r/scale * Math.cos(lat) * Math.sin(lon);
    let z = r/scale * Math.sin(lat);
  ...
  function setBODYRotation(cond) {
    ...
    let BODYframe = scene.getObjectByName('BODY');

    //get initial quaternions
    let { e0, e1, e2, e3 } = cond;
  ...
  */
  function createVelVect(NEDframe, cond) {
    ...
    let dir = new THREE.Vector3(u, v, w);
    dir.normalize(); //has to be normalized
    let origin = new THREE.Vector3( 0, 0, 0 );
    let length = vInf/100000;

    //create velocity vector and assign a name to it
    let arrowHelper = new THREE.ArrowHelper(dir, origin, length, 0xffffff, 0.006, 0.002);
  ...
  function setVelocityVector(cond) {
    ...
    let velVector = scene.getObjectByName('VELOCITY');
  ...
  function updateVelVect(time, { spline }) {
    ...
    let length = spline.tVSvel.at(time)/100000;
    let u = spline.tVSxVel.at(time);
    let v = spline.tVSyVel.at(time);
    let w = spline.tVSzVel.at(time);
  ...
  function defineAnim({ arr }) {
    ...
    //ROTATION NED
    let quaternionNKF  = new THREE.QuaternionKeyframeTrack('.quaternion', arr.t, arr.rotN);

    //POSITION NED
    let positionKF = new THREE.VectorKeyframeTrack('.position', arr.t, arr.pos);

    //ROTATION BODY
    let quaternionBKF = new THREE.QuaternionKeyframeTrack('.children[1].quaternion', arr.t, arr.rotB); //BODYframe is children[1] of NEDframe

    //create an animation sequence with the track
    let duration = arr.t[arr.t.length-1];
    let clip = new THREE.AnimationClip('entry', duration, [quaternionNKF, positionKF, quaternionBKF]);
  ...
  function updateView() {
    ...
    let delta = clock.getDelta(); //get time passed from last callback

    if (mixer) {
      let currentTime = clipAction.time; //get the current real simulation time
...
function setupGUI() {
  //initialize GUI and set its width
	let gui = new dat.GUI({width: 300}); //specify width of gui

  let cond = traj.cond; //contains initial conditions
  ...
  //MASS
  let mass = gui.add(lander, 'mass', 10, 50000).name('Mass [kg]');
  //VELOCITY
  let vel = gui.add(cond, 'vInf', 1000, 10000).name('Velocity [m/s]');
  vel.onChange(() => G.setVelocityVector(cond));
  //COMx PAYLOAD
  ...
  //number of divisions
  let nop = folder.add(lander.mesh, 'nop', 10, 50).step(1).name('Mesh Divisions');
  nop.onChange(() => {lander.defineMesh(); G.setLander(lander, cond);});
  //diameter
  let dia = folder.add(lander,'diameter', 1, 30).name('Diameter [m]');
  dia.onChange(() => {lander.defineMesh(); G.setLander(lander, cond);});
  //nose radius
  let nRad = folder.add(lander,'rNose', 0.1, 10).name('Nose Radius [m]');
  nRad.onChange(() => {lander.defineMesh(); G.setLander(lander, cond);});
  //angle
  let theta = folder.add(lander, 'sphereConeAngleDeg', 10, 80).name('Cone Angle [deg]');
  ...
  //altitude
	let alt = folder.add(cond, 'h', 100000, 200000).step(5000).name('Altitude [m]');
  alt.onChange(() => {G.setLanderPosition(lander, cond); liveOutputs.init();});
  //latitude
  let lat = folder.add(cond,'latDeg', -90, 90).step(5).name('Latitude [deg]');
  lat.onChange(() => {G.setLanderPosition(lander, cond); G.setNEDRotation(cond);});
  //longitude
  let lon = folder.add(cond,'lonDeg', 0, 360).step(5).name('Longitude [deg]');
  lon.onChange(() => {G.setLanderPosition(lander, cond); G.setNEDRotation(cond);});

  //ANGLES
  folder = gui.addFolder('ANGLES');
  //flight path angle
  let FPA = folder.add(cond, 'FPADeg', 0, 90).name('FPA [deg]');
  FPA.onChange(() => {G.setVelocityVector(cond); G.setBODYRotation(cond);});
  //azimuth
  let azi = folder.add(cond, 'aziDeg', -180, 180).name('Azimuth [deg]');
  azi.onChange(() => {G.setVelocityVector(cond); G.setBODYRotation(cond);});
  //angle of attack
  let AoA = folder.add(cond, 'alphaDeg', -80, 80).name('Angle of Attack [deg]');
  ...
  //ATMOSPHERES
  let atmo = gui.add(traj, 'current',{Pathfinder: 'pathfinder',Curiosity: 'curiosity',Opportunity: 'opportunity',Phoenix: 'phoenix', Schiaparelli: 'schiaparelli', Spirit: 'spirit'}).name('Atmosphere');

  //TRAJECTORY function - save function
  let save = gui.add(results, 'run').name('Get Trajectory');
  ...
  //stop
  let stop = folder.add(G, 'stopAnim').name('Stop');
  stop.onFinishChange(() => G.setVelocityVector(cond));
  //pause
  folder.add(G, 'pauseAnim').name('Pause');

  // //HEATSHIELD PROPERTIES
  // folder = gui.addFolder('HEATSHIELD PROPERTIES');
  // //density
  // let hsDensity = folder.add(lander, 'hsDensity', 100, 500).name('Density [kg/m3]');
  // //thickness
  // let hsThickness = folder.add(lander, 'hsThickness', 0, 0.2).name('Thickness [m]');
